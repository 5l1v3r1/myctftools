...
function filter_invisible($str){
    for($i=0;$i<strlen($str);$i++){
        $ascii = ord($str[$i]);
        if( $ascii < 32){ //有中文这里要修改为不检测>126的即去掉 || $ascii>126
...
function filter_attack_keyword($str){
    ...
    if(preg_match("/base64_decode|eval\(|assert\(|file_get_contents(|file_put_contents(/i", $str)){
...
function filter_dangerous_words($str){
    ...
    $str = str_replace("assert"," ",$str); 
    //过滤常用的读写文件和flag的关键字
    $str = str_replace("file_put_contents"," ",$str);
    $str = str_replace("file_get_contents"," ",$str);
   return $str;
}
/*
...
function write_attack_log($alert){
    ...
    fclose($ffff);
    if($alert == 'GETFLAG'){
        echo "CTF{Th1s_1s_th3_fla9}"; //如果请求带有flag关键字，显示假的flag。（2333333）
    }else{
        sleep(15); //拦截前延时15秒
    }
    exit(0);
